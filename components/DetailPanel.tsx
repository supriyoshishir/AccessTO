...
export default function DetailPanel({ place }: DetailPanelProps) {
  ...
  if (!place) {
    return <p className="text-slate-600">Select a record to see its details.</p>;
  }
  ...
  return (
    ...
    >
      ...
      {(place.lat === null || place.lng === null) && (
        <p className="mt-1 text-sm text-slate-600">Location unavailable</p>
      )}
      {fieldEntries.length > 0 && (
        <dl className="mt-4 grid grid-cols-[max-content_1fr] gap-x-4 gap-y-2 text-sm">
          {fieldEntries.map(([key, value]) => (
            <div key={key} className="contents">
              <dt className="font-medium text-slate-700">{prettifyLabel(key)}</dt>
